...
function Map(props: MapProps) {
  ...
  async function _onLoad() {
    ...
    if (import.meta.env.VITE_PUBLIC_APP_ENV === "testnet" && !id) {
      mapRef.current.easeTo({
        center: [viewport.longitude, viewport.latitude],
        zoom: 13,
        duration: 500,
    ...
    if (id) {
      const parcelId = typeof id === "string" ? id : id[0];
    ...
    <>
      ...
      <Col sm={interactionState != STATE.VIEWING ? "9" : "12"} className="px-0">
        <ReactMapGL
          ...
          mapboxAccessToken={import.meta.env.VITE_PUBLIC_REACT_APP_MAPBOX_TOKEN}
        ...
        >
          ...
          {!isMobile || showSearchBar ? (
            <Geocoder
              mapboxAccessToken={
                import.meta.env.VITE_PUBLIC_REACT_APP_MAPBOX_TOKEN ?? ""
              }
              position="top-right"
