...
import { LoginState } from "../../IndexPage";
...
export default function ParcelInfo(props: ParcelInfoProps) {
  const [mudSetup, setMUDSetup] = React.useState<any | null>(null);
  React.useEffect(() => {
    (async () => {
  ...
  return (
    <MUDProvider value={mudSetup}>
      <ParcelInfoInner {...props} />
    </MUDProvider>
  );
}

function ParcelInfoInner(props: ParcelInfoProps) {
  const {
    account,
    signer,
    smartAccount,
        ...
        <Row className="m-0 mt-2 mt-sm-3 pb-1 pb-lg-5">
          <Col className="p-0">
            <div className="d-flex flex-column gap-1 gap-sm-3">
              ...
              <div className="text-truncate">
                <span className="fw-bold">World Address: {worldAddress}</span>
              </div>
              <br />
