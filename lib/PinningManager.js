...
export function usePinningManager(
  ...
  React.useEffect(() => {
    if (!ceramic || !ipfs || !firebasePerformance || !parcelIndexManager) {
    ...
    async function setupManager() {
      ...
      const bucket = new GeoWebBucket(parcelIndexManager, ipfs);

      const index = parcelIndexManager
        ? parcelIndexManager.getStreamContent()
        : null;

      const pinsetStreamId = index
        ? index[parcelIndexManager.dataStore.getDefinitionID("geoWebPinset")]
        : null;
    ...
    setupManager();
  }, [ceramic, ipfs, firebasePerformance, parcelIndexManager]);
