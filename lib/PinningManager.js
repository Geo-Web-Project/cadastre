...
export class PinningManager {
  constructor(geoWebBucket) {
    this._geoWebBucket = geoWebBucket;
  }

  async pinCid(cid, updatePinningData) {
    updatePinningData({
      [cid]: {
        status: "queued",
      },
...
export function usePinningManager(ceramic) {
  ...
  React.useEffect(() => {
    ...
    async function setupManager() {
      ...
      const _pinningManager = new PinningManager(bucket);
      console.log(_pinningManager);
