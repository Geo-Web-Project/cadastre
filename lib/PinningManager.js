...
export class PinningManager {
  ...
  async getStorageUsed() {
    const bucketRootPath = await this.getBucketRootPath();
    const objectStat = await this._ipfs.object.stat(bucketRootPath);
    return objectStat.CumulativeSize;
  }

  async archive() {
    await this._geoWebBucket.archive();
    console.log(await this._geoWebBucket.listArchives());
  }
}
