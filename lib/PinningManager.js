...
export class PinningManager {
  constructor(geoWebBucket, ipfs, firebase) {
...
export function usePinningManager(ceramic, ipfs, firebase) {
  const [pinningManager, setPinningManager] = React.useState(null);

  React.useEffect(() => {
    if (!ceramic || !ipfs || !firebase) {
      return;
    }
    ...
    async function setupManager() {
      ...
      const _pinningManager = new PinningManager(bucket, ipfs, firebase);
      setPinningManager(_pinningManager);
    }

    setupManager();
  }, [ceramic, ipfs, firebase]);
