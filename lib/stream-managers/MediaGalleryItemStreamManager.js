  ...
  async createOrUpdateStream(content) {
    ...
    if (newStreamId) {
      ...
      await this.mediaGalleryStreamManager.createOrUpdateStream(
        prevState.concat(newStreamId.toString())
      );
    }
...
export function useMediaGalleryItemData(mediaGalleryStreamManager) {
  ...
  React.useEffect(() => {
    ...
    async function updateItems() {
      // FIXME: Multiquery is returning documents that fail on .change()
      ...
      const mediaGalleryIds = await mediaGalleryStreamManager.getStreamContent();
      const mediaGalleryItemManagers = await Promise.all(
        mediaGalleryIds.map(async (streamId) => {
          const _mediaGalleryItemStreamManager = new MediaGalleryItemStreamManager(
            mediaGalleryStreamManager.ceramic
          );
          _mediaGalleryItemStreamManager.setMediaGalleryStreamManager(
          ...
          await _mediaGalleryItemStreamManager.setExistingStreamId(streamId);
          return _mediaGalleryItemStreamManager;
        })
      );
      const mediaGalleryItems = mediaGalleryItemManagers.reduce(
        (result, manager) => {
          result[manager.getStreamId()] = manager;
          return result;
        },
        {}
      );
