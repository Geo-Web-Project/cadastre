...
export class GeoWebBucket {
  constructor(parcelIndexManager, ipfs) {
    this._parcelIndexManager = parcelIndexManager;
    this._ipfs = ipfs;
    this.bucketRoot;
  ...
  async fetchOrProvisionBucket(queueDidFail) {
    const pinsetIndex = await this._parcelIndexManager.get("geoWebPinset");
    if (pinsetIndex) {
  ...
  async reset() {
    ...
    await this._parcelIndexManager.set("geoWebPinset", {
  ...
  async triggerPin() {
    ...
    const index = await this._parcelIndexManager.getIndex();
    const pinsetRecordID = StreamID.fromString(
      index[this._parcelIndexManager.dataStore.getDefinitionID("geoWebPinset")]
    );
    const pinsetStream = await this._parcelIndexManager.ceramic.loadStream(
      pinsetRecordID
    );
