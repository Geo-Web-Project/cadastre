...
export class GeoWebBucket {
  ...
  async fetchOrProvisionBucket() {
    ...
    if (pinsetIndex) {
      ...
      console.log(`this.bucketRoot: ${JSON.stringify(this.bucketRoot)}`);
  ...
  async isPinned(name) {
    if (!this.latestPinnedRoot) {
      return false;
    }

    const links = await this._ipfs.object.links(this.latestPinnedRoot);
    return links.filter((v) => v.Name == name).length > 0;
  }
